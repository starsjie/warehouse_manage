...
public class ProductServiceImpl implements ProductService {
    ...
    public Result saveProduct(Product product) {
        //判断商品型号是否存在
        Product proNum = productMapper.findProductByNum(product.getProductNum());
        if (proNum!=null){
    ...
    public Result deleteProductByIds(List<Integer> productIdList) {
        int i = productMapper.deleteProductById(productIdList);
        if(i>0){
    ...
    public Result updateProduct(Product product) {
        ...
        Product prodNum = productMapper.findProductByNum(product.getProductNum());
        if (prodNum!=null && !prodNum.getProductId().equals(product.getProductId())){
            return Result.err(Result.CODE_ERR_BUSINESS,"修改后的商品型号已存在");
        }
         /*
          处理商品上传的图片的访问地址:
